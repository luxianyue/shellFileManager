...
static int create_subprocess(JNIEnv *env, const char *cmd, char *const argv[], char *const envp[], int masterFd)
{
    ...
    if(pid == 0){
        ...
        dup2(pts, 1);
        dup2(pts, 2);
        //int error_fd = open("/data/data/com.lu.filemanager2/error", O_RDWR);
        //LOGE("-------->devname:%s  ,%d",devname,error_fd);
        //dup2(error_fd, 2);
...
JNIEXPORT void JNICALL Java_com_lu_shell_TermExec_sendSignal(JNIEnv *env, jobject clazz,
    jint procId, jint signal)
{
    kill(procId, signal);
}

JNIEXPORT jint JNICALL Java_com_lu_shell_TermExec_waitFor(JNIEnv *env, jclass clazz, jint procId) {
    int status;
...
JNIEXPORT jint JNICALL Java_com_lu_shell_TermExec_createSubprocessInternal(JNIEnv *env, jclass clazz,
